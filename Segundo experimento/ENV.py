...
class MujocoEnvCNN(gym.Env):
    ...
    def __init__(self, scene_path=None, movement_pattern = 'down', obs_shape = (300, 300, 3), num_frames = 1000000, cfg=None):
        ...
        self.env_scene = "cubemedium_pass_1_s7_scene.xml"
        self.experiment = 2
    ...
    def calculate_force_projection(self):
        ...
        # Upward direction vector (Z-axis)
        up_vector = np.array([0, 0, 1])
        
        # Calculate dot product (projection)
        projection = np.dot(normalized_force, up_vector)
        
        # Convert to percentage (0 to 1)
        # Clamp to avoid negative values (forces pointing downward)
        force_projection = max(0.0, projection)
    ...
    def reward(self):
        ...
        # FORCE MEASUREMENT
        # In this part, forces will be measured on fingertips and distal phalanges


        total_force = 0
        final_vector = np.zeros(3)

        #Heavily reward contact with thumb + a combination of other fingers
        #if fingers_contact['th'] and (fingers_contact['ff'] or fingers_contact['mf'] or fingers_contact['rf']):
        #    reward += 5
    ...
    def _get_obs(self):
        ...
        # Get target orientation (as quaternion)
        target_orientation = self.data.xquat[self.obj_id]
        # Get finger positions
        finger_tip_positions = self.data.xpos[[self.ff_tip_id, self.mf_tip_id, self.rf_tip_id, self.th_tip_id]].flatten()

        # Get hand orientation
        hand_orientation = self.data.xquat[self.hand_id]

        #Get relative orientation between hand and object
        #hand_to_object_orientation = quaternion_multiply(quaternion_conjugate(hand_orientation), target_orientation)
        hand_to_object_orientation = target_orientation - hand_orientation


        # Stack all observations into a single vector
        #obs = np.concatenate([normalized_forces, fingertip_positions, fingertip_velocities
        #                      , object_position, target_orientation, hand_orientation, hand_to_object_orientation])
        assert not np.any(np.isnan(normalized_forces)), "NaN in normalized forces"
        # Must be 35, 1 D array
        assert normalized_forces.shape == (12,), f"Expected forces shape (12,), got {normalized_forces.shape}"
        assert finger_tip_positions.shape == (12,), f"Expected finger_tip_positions shape (12,), got {finger_tip_positions.shape}"
        
        combined = np.concatenate([normalized_forces, fingertip_positions, object_position, target_orientation, hand_orientation, hand_to_object_orientation])
        assert combined.shape[0] == 12+12+3+4+4+4, f"Expected combined shape {(12+12+3+4+4+4,)}, got {combined.shape}"
